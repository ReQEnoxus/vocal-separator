...
def main():
    ...
    artifact_dir = f"artifacts/{train_id}"
    os.mkdir(artifact_dir)
    dataframe_train = pd.read_csv("vad_meta_train.csv", names=['track', 'mask'], header=None)
    dataframe_val = pd.read_csv("vad_meta_val.csv", names=['track', 'mask'], header=None)
    ...
    sgdOpt = tf.keras.optimizers.SGD(lr=0.005, decay=1e-6, momentum=0.87, nesterov=True)

    model.compile(loss=tf.keras.losses.binary_crossentropy, optimizer=sgdOpt, metrics=['accuracy'])
    # model.load_weights("fixed_weights-improvement-17-0.81.hdf5")
    ...
    dataset_train = tf.data.Dataset.from_generator(lambda: generate_data(dataframe_train, BATCH_SIZE),
                                                   ...
                                                   output_shapes=((None, 1025, 9, 1), (None, 1)))
